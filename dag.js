...
class Dag {
	constructor(dagData,visibleNodes) {
		...
		const traverseNode=node=>{
			...
			for (let parentNode in this.allParents[node]) {
				...
				for (let ancestorNode in ancestorDistances[parentNode]) {
					...
					addAncestorDistance(node,ancestorNode,parentDistance+!!visibleNodes[parentNode])
				}
			}
		}
